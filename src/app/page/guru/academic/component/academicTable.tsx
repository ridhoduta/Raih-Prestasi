...
export const AcademicTable = ({ students, onSaveScores, yearId, semester }: AcademicTableProps) => {
      ...
      <table className="w-full text-left border-collapse">
        <thead className="bg-gray-50 border-b border-gray-100">
          <tr>
            ...
            <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Nilai Akademik</th>
            ...
            <tr key={student?.id || Math.random()} className="hover:bg-gray-50/50 transition-colors">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {student?.achievements?.map((ach: any) => (
                    <span key={ach.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-100">
                      <Trophy size={10} />
                      {ach.competitionName} ({ach.result})
                    </span>
                  ))}
                </div>
              </td>
              <td className="px-6 py-4">
                <div className="flex flex-wrap gap-1">
                  {student?.academicScores && student.academicScores.length > 0 ? (
                    student.academicScores.map((score: any) => (
                      <span key={score.id} className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-blue-50 text-blue-700 border border-blue-100">
                        {score.subject}: {score.score}
                      </span>
                    ))
                  ) : (
                    <span className="text-xs text-gray-400 italic">Belum ada nilai</span>
                  )}
                </div>
              </td>
              <td className="px-6 py-4 text-right">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl hover:bg-emerald-100 transition-all text-xs font-bold border border-emerald-100 shadow-sm"
                >
                  <FileText size={14} />
                  Input Nilai
                </button>
